...
- [`adr/2026-05-18-node-task-worker-threads.md`](adr/2026-05-18-node-task-worker-threads.md) — Node `worker_threads`-backed `Task.runRegistered` for blocking `await()`. Read before touching `wvlet.uni.control.Task`'s JS impl; covers the `import.meta.url` bundle-URL discovery, the test-bridge tolerance, the `@JSExportTopLevel val` eager-bootstrap pattern, and the `js.Dynamic.global.updateDynamic` → bare-identifier-assignment optimizer trap.
