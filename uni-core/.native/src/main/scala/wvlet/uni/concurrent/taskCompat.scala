...
/**
  * Scala Native uses the same JDK threading primitives as JVM, so this mirrors the JVM impl. The
  * only intentional difference is `Thread.interrupt()` — Scala Native supports it, but its
  * semantics against libc blocking syscalls are weaker than HotSpot's. Bodies that need unblocking
  * on cancel should still observe `isCancelled` at loop checkpoints.
  */
